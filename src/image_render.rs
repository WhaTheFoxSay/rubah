...
pub fn render_image_to_lines(img_bytes: &[u8], target_width: u32, max_height_rows: u32) -> Option<Vec<Line<'static>>> {
    ...
    // High-definition wide column width calculation (45 to 70 columns wide for maximum clarity)
    let available_cols = target_width.clamp(45, 70);
    let img_aspect = orig_w as f32 / orig_h as f32;

    // Dual-pixel vertical resolution engine (2 vertical pixels per cell for 2x crispness)
    let target_pixel_w = available_cols;
    let target_pixel_h = ((available_cols as f32 / img_aspect)).round() as u32;
    let max_pixel_h = (max_height_rows.clamp(14, 24)) * 2;
    ...
    // Lanczos3 high-definition resampling
