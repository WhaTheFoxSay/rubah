...
pub fn render_image_to_lines(img_bytes: &[u8], target_width: u32, max_height_rows: u32) -> Option<Vec<Line<'static>>> {
    ...
    {
        // On Windows CMD/PowerShell (conhost.exe), solid block '█' with FG-only TrueColor
        // eliminates conhost's black line spacing gaps caused by half-block BG rendering.
        let available_cols = target_width.clamp(44, 52);
        let font_aspect = 0.50; // Windows console font aspect ratio multiplier
        let target_pixel_w = available_cols;
        let target_pixel_h = ((available_cols as f32 / img_aspect) * font_aspect).round() as u32;
        let final_pixel_h = target_pixel_h.clamp(12, max_height_rows.clamp(12, 18));
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        let available_cols = target_width.clamp(45, 65);
        let target_pixel_w = available_cols;
