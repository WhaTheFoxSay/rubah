...
step "Binary & symlink" "~/.local/bin/baca terhapus"

rm -rf "$HOME/.config/rubah"
step "Config & database" "~/.config/rubah terhapus"

rm -rf "$HOME/.cache/rubah" 2>/dev/null || true
step "Cache & temp files" "~/.cache/rubah terhapus"
...
echo -e "  ${ORANGE}Sampai jumpa kembali! 🦊${RESET}\n"
